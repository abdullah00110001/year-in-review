...
import { ArrowLeft, Search, Loader2, Smartphone, CheckCircle2 } from 'lucide-react';
...
export function BlockAppsPage({ onBack }: BlockAppsPageProps) {
      ...
      try {
        ...
        if (isNative) {
          ...
          try {
            ...
            // Fallback to recently-used apps
          ...
          // Web preview: show a few mock entries
          setApps([
            ...
            { packageName: 'com.zhiliaoapp.musically', appName: 'TikTok', isSystem: false },
            { packageName: 'com.twitter.android', appName: 'X', isSystem: false },
          ]);
        }
      } finally {
        setLoading(false);
  ...
  const persist = async (next: Set<string>) => {
    ...
    if (isNative) {
      try { await ShieldPlugin.blockApps({ apps: list }); } catch (e) {
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-lg border-b border-border">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">{blocked.size} app{blocked.size !== 1 && 's'} blocked</p>
          </div>
        </div>
        <div className="px-4 pb-3 space-y-2">
          ...
          <label className="flex items-center justify-between text-xs text-muted-foreground px-1">
      ...
      {loading ? (
        ...
        <div className="px-4 py-3 space-y-2">
          {sortedFiltered.length === 0 ? (
            <Card className="border-dashed"><CardContent className="p-8 text-center text-muted-foreground">No apps match your search.</CardContent></Card>
          ) : (
            sortedFiltered.map(app => {
              const isBlocked = blocked.has(app.packageName);
              return (
                <Card key={app.packageName} className={isBlocked ? 'border-rose-500/50 bg-rose-500/5' : ''}>
                  <CardContent className="p-3 flex items-center justify-between gap-3">
                    ...
                    <Switch checked={isBlocked} onCheckedChange={() => toggle(app.packageName)} className="data-[state=checked]:bg-rose-500" />
                  </CardContent>
                </Card>
              );
            })
